...
function matchesFilters(item) {{
  ...
  if (activeChild !== 'All') {{
    ...
    if (!itemMatchesChild(item, child)) return false;
  }} else {{
    // "All" means "relevant to at least one of my kids" -- not literally
    // every year group the school has. A genuinely whole-school item
    // (tagged "All") still matches every child's own check, so nothing that
    // should be visible gets hidden; this only filters out content tied
    // exclusively to a year group neither child is in (e.g. Kindergarten,
    // Year 1-4, Year 6-7, Year 9-13).
    const realChildren = CHILDREN.filter(c => c.id !== 'All');
    const matchesAny = realChildren.some(c => itemMatchesChild(item, c));
...
if __name__ == "__main__":
    main()def build_html(rows, email_log_rows=None):
    email_log_rows = email_log_rows or []
    data_json = json.dumps(rows, default=str)
    email_log_json = json.dumps(email_log_rows, default=str)
    updated = datetime.now().strftime('%d %b %Y, %H:%M')
